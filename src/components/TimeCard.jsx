    ...
    return (
        <div className='flex flex-col md:flex-row 3xl:h-4/12 gap-10 justify-center'>
            ...
            <motion.div className='md:w-6/12 flex flex-col justify-around self-center gap-3 md:gap-5 lg:gap-8' variants={onLoadVariant(-50, 0, 0.2)} initial="hidden" whileInView="show" >
                ...
                <div>
                    <div className={`text-lg 3xl:text-xl ${styles.descText}`}><span className='text-sectext text-sm 2xl:text-lg pr-2 2xl:pr-5'>
                        Start Date:</span>{monthName}, {timeData.dateS.getDate()}, {timeData.dateS.getFullYear()}
                    </div>
                    {timeData.dateF.getFullYear() == 2000 ?
                        <div className={`text-lg 3xl:text-xl ${styles.descText}`}>
                            <span className='text-sectext text-sm 2xl:text-lg pr-2 2xl:pr-5'>Finish Date:</span>
                            Not finished yet
                        </div> :
                        <div className={`text-lg 3xl:text-xl ${styles.descText}`}>
                            <span className='text-sectext text-sm 2xl:text-lg pr-2 2xl:pr-5'>Finish Date:</span>
                            {monthName2}, {timeData.dateF.getDate()}, {timeData.dateF.getFullYear()}
                        </div>}
                </div>
